...
The REST API is the interface to access the Dragon tasks system.
...
the error message for tasks that error. For successes, ``result`` is normally
...
With ``files`` containing URLs of file outputs (for tasks that output files, e.g.
thumbnails), and ``values`` containing the rest of the task output values.
